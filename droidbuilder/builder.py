...
def _compile_runtime_package(package_name, package_config, runtime_package_source_path, install_dir, env_obj, config):
    ...
    site_packages_dir = os.path.join(install_dir, "lib", f"python{major_minor}", "site-packages")
    ...
    python_include_dir = os.path.join(install_dir, "include", f"python{major_minor}")
...
def _copy_assets_to_android_app(build_path, archs, install_path):
    ...
    for arch in archs:
        # Copy Python assets
        install_dir = os.path.join(install_path, arch)
        dest_depend_dir = os.path.join(assets_dir,"local", arch)
        
        if not os.path.exists(install_dir):
            logger.error(f"Error: Compiled Python for {arch} not found at {install_dir}. Please ensure Python was built successfully for this architecture.")
            return False

        try:
            shutil.copytree(install_dir, dest_depend_dir, dirs_exist_ok=True)
            logger.info(f"    - Copied Python assets for {arch} to {dest_depend_dir}")
        except (shutil.Error, OSError) as e:
            logger.error(f"Error copying Python assets for {arch} from {install_dir} to {dest_depend_dir}: {e}")
            logger.info("Please check directory permissions and ensure enough disk space is available.")
            return False
    
    return True

def _copy_user_project_files(build_path, root_dir):
    """Copy user's Python application code to Android app assets."""
    logger.info("  - Copying user's Python code to Android app...")

    user_project_files = os.path.join(build_path, "app", "src", "main", "assets", "source_code")
    try:
        os.makedirs(user_project_files, exist_ok=True)
    except OSError as e:
        logger.error(f"Error creating user Python assets directory {user_project_files}: {e}")
        return False
    ...
    try:
        ...
        shutil.copytree(source_dir, user_project_files, dirs_exist_ok=True, ignore=ignore_patterns)
        
        logger.success(f"  - Copied user's project files from {source_dir} to {user_project_files}")
    except (shutil.Error, OSError) as e:
...
def build_android(config, verbose):
    ...
    try:
        ...
        for package_name, package_version in runtime_packages:
            ...
            for arch in archs:
                install_dir = os.path.join(install_path, arch)
                if not _compile_runtime_package(name, {}, runtime_package_source_path, install_dir, env_map[arch], config):
        ...
        if not _copy_user_project_files(build_path, root_dir):
